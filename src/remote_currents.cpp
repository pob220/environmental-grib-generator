...
std::filesystem::path TemporarySibling(const std::filesystem::path& output,
  ...
  auto path = output;
  path += "." + std::to_string(::getpid()) + suffix;
  return path;
}
}  // namespace
...
DirectCurrentResult DownloadMarineIe(const std::filesystem::path& output,
  ...
  if (raw_bytes.empty()) throw ValidationError("Marine.ie download returned empty response");
  ...
  try {
    ...
    const auto u_count = inspection["current_component_counts"]["u_49"].asUInt64();
    const auto v_count = inspection["current_component_counts"]["v_50"].asUInt64();
    if (u_count == 0 || v_count == 0 || u_count != v_count) {
      throw ValidationError("Marine.ie GRIB does not contain balanced u/v current components");
    }
    std::filesystem::rename(clean_path, output);
    std::error_code ignored;
    std::filesystem::remove(raw_path, ignored);
    return {"marine_ie_irish_sea", output, normalized.raw_byte_count,
            normalized.clean_byte_count, normalized.skipped_byte_count,
            inspection};
  } catch (...) {
    std::error_code ignored;
    std::filesystem::remove(raw_path, ignored);
