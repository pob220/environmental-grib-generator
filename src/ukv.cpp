...
std::vector<int> Dims(int file, int variable) {
  int count = 0; Nc(nc_inq_varndims(file, variable, &count), "reading UKV dimensions");
  std::vector<int> result(static_cast<std::size_t>(count));
  Nc(nc_inq_vardimid(file, variable, result.data()), "reading UKV dimension ids");
  return result;
}
std::size_t DimSize(int file, int dim) {
  std::size_t size = 0; Nc(nc_inq_dimlen(file, dim, &size), "reading UKV dimension size"); return size;
}
std::string AttributeText(int file, int variable, const char* name) {
  ...
  Nc(nc_get_att_text(file, variable, name, value.data()), "reading UKV text attribute");
  return value;
}
double AttributeDouble(int file, int variable, const char* name, double fallback) {
  double value = fallback;
  return nc_get_att_double(file, variable, name, &value) == NC_NOERR ? value : fallback;
}
int FindCoordinate(int file, const char* standard_name, const char* fallback) {
  int variables = 0; Nc(nc_inq_nvars(file, &variables), "reading UKV variable count");
  ...
  throw ValidationError(std::string("UKV coordinate not found: ") + standard_name);
}
std::vector<double> Read1D(int file, int variable) {
  const auto dims = Dims(file, variable);
  if (dims.size() != 1) throw ValidationError("UKV projected coordinate must be one-dimensional");
  std::vector<double> result(DimSize(file, dims[0]));
  Nc(nc_get_var_double(file, variable, result.data()), "reading UKV projected coordinate");
  return result;
}
...
struct ProjectedField {
  ...
  double lat0{}, lon0{}, false_easting{}, false_northing{}, semi_major{}, semi_minor{};
};

ProjectedField ReadProjectedField(const std::filesystem::path& path) {
  NcFile file(path);
  const int x_id = FindCoordinate(file.id(), "projection_x_coordinate", "projection_x_coordinate");
  const int y_id = FindCoordinate(file.id(), "projection_y_coordinate", "projection_y_coordinate");
  const auto x = Read1D(file.id(), x_id), y = Read1D(file.id(), y_id);
  int variables = 0; Nc(nc_inq_nvars(file.id(), &variables), "reading UKV variable count");
  ...
  for (int id = 0; id < variables; ++id) {
    ...
    if (!grid_mapping.empty() && dims.size() >= 2) {
      data = id;
      Nc(nc_inq_varid(file.id(), grid_mapping.c_str(), &mapping), "finding UKV grid mapping");
      break;
    }
  }
  if (data < 0 || mapping < 0) throw ValidationError("UKV source lacks a projected data variable/grid mapping");
  if (AttributeText(file.id(), mapping, "grid_mapping_name") != "lambert_azimuthal_equal_area")
    throw ValidationError("unsupported UKV projection; expected Lambert azimuthal equal area");
  ...
  count[dims.size() - 2] = y.size(); count[dims.size() - 1] = x.size();
  std::vector<double> values(x.size() * y.size());
  Nc(nc_get_vara_double(file.id(), data, start.data(), count.data(), values.data()), "reading UKV data");
  double fill = std::numeric_limits<double>::quiet_NaN();
  const bool have_fill = nc_get_att_double(file.id(), data, "_FillValue", &fill) == NC_NOERR ||
  ...
  return {x, y, values, mask,
          AttributeDouble(file.id(), mapping, "latitude_of_projection_origin", 54.9),
          AttributeDouble(file.id(), mapping, "longitude_of_projection_origin", -2.5),
          ...
          AttributeDouble(file.id(), mapping, "semi_minor_axis", 6356752.314245179)};
}

#ifdef ENVIRONMENTAL_GRIB_HAVE_PROJ
std::pair<double, bool> Interpolate(const ProjectedField& field, double x, double y) {
  ...
  const auto x0 = static_cast<std::size_t>(bx[0]), x1 = static_cast<std::size_t>(bx[1]);
  const auto y0 = static_cast<std::size_t>(by[0]), y1 = static_cast<std::size_t>(by[1]);
  const std::array<std::size_t, 4> i{y0 * field.x.size() + x0, y0 * field.x.size() + x1,
                                     y1 * field.x.size() + x0, y1 * field.x.size() + x1};
  for (auto index : i) if (field.mask[index]) return {0.0, false};
  const double lower = field.values[i[0]] * (1 - bx[2]) + field.values[i[1]] * bx[2];
  const double upper = field.values[i[2]] * (1 - bx[2]) + field.values[i[3]] * bx[2];
...
class Projection {
 public:
  ...
  std::pair<double, double> Forward(double lon, double lat) const {
    const auto coordinate = proj_trans(projection_, PJ_FWD, proj_coord(lon, lat, 0, 0));
    if (proj_errno(projection_)) throw ValidationError("PROJ failed UKV coordinate transformation");
    return {coordinate.xy.x, coordinate.xy.y};
  }
 private: PJ_CONTEXT* context_{}; PJ* projection_{};
};
#endif

std::vector<std::string> Cycles(const UkvRequest& request, TimePoint now) {
  if (request.cycle != "auto") {
    if (!request.date) throw ValidationError("UKV explicit cycle requires date");
    return {*request.date + "T" + request.cycle + "00Z"};
  }
  const std::array<const char*, 8> cycles{"21", "18", "15", "12", "09", "06", "03", "00"};
  ...
  for (int day = 0; day < std::max(1, request.max_auto_days); ++day) {
    ...
    for (const char* cycle : cycles) result.push_back(date + "T" + cycle + "00Z");
  }
  return result;
}
}  // namespace
...
std::vector<int> UkvForecastHours(int hours, int step) {
  if (hours < 0 || hours > 120) throw ValidationError("UKV hours must be between 0 and 120");
  if (step == 1) {
    std::vector<int> result;
    for (int hour = 0; hour <= std::min(hours, 54); ++hour) result.push_back(hour);
    for (int hour = 57; hour <= hours; hour += 3) result.push_back(hour);
    return result;
  }
  if (step == 3 && hours % 3 == 0) {
    std::vector<int> result; for (int hour = 0; hour <= hours; hour += 3) result.push_back(hour); return result;
  }
  throw ValidationError("UKV step-hours must be 1 or 3, with 3-hour requests divisible");
}

std::string UkvSourceKey(const std::string& cycle, int hour,
                         const std::string& field) {
  const auto time = ParseUtcDateTime(cycle.substr(0, 4) + "-" + cycle.substr(4, 2) + "-" +
                                     cycle.substr(6, 2) + "T" + cycle.substr(9, 2) + ":00:00Z") +
                    std::chrono::hours(hour);
  char lead[16]; std::snprintf(lead, sizeof(lead), "PT%04dH00M", hour);
...
WeatherGenerateResult GenerateUkv(const UkvRequest& request, HttpGet download,
  ...
  if (!kUkvDomain.Contains(request.bbox)) throw ValidationError("UKV bbox is outside UK/Ireland domain");
  if (!UkvProjectionAvailable()) throw UnsupportedSourceError("UKV requires the native PROJ library");
  if (request.grid_spacing_deg <= 0.0) throw ValidationError("UKV grid spacing must be positive");
  ...
  const TimePoint current = now.value_or(
      std::chrono::time_point_cast<std::chrono::seconds>(std::chrono::system_clock::now()));
  const auto candidates = Cycles(request, current);
  if (request.dry_run) {
    return {"ukmo_ukv", "Met Office UKV 2 km forecast", "uk_deterministic_2km",
            GFSCycle{candidates.front().substr(0, 8), candidates.front().substr(9, 2)},
            request.bbox, hours, request.output, 0, 0,
  ...
  for (const auto& cycle : candidates) {
    files.clear(); urls.clear();
    bool complete = true;
    try {
      for (int hour : hours) for (const auto& [short_name, token] : kFields) {
        const auto key = UkvSourceKey(cycle, hour, token);
        const auto url = std::string(kEndpoint) + key;
        Json::Value detail; detail["cycle"] = cycle; detail["hour"] = hour; detail["field"] = token;
        if (progress) progress("downloading Met Office UKV source", detail);
        const auto bytes = download(url, request.timeout_seconds);
        if (bytes.empty()) throw ValidationError("UKV source download was empty");
        auto path = std::filesystem::temp_directory_path() /
                    ("environmental-ukv-" + std::to_string(::getpid()) + "-" +
  ...
  for (int hour : hours) {
    ...
    for (const auto& name : {"prmsl", "2t", "10u", "10v"}) {
      values[name].resize(grid.size()); masks[name].resize(grid.size());
    }
    for (std::size_t y = 0; y < grid.ny(); ++y) for (std::size_t x = 0; x < grid.nx(); ++x) {
      const auto [px, py] = projection.Forward(grid.longitudes[x], grid.latitudes[y]);
      const auto p = Interpolate(pressure, px, py), t = Interpolate(temperature, px, py);
      const auto u = Interpolate(source_u, px, py), v = Interpolate(source_v, px, py);
      const auto index = y * grid.nx() + x;
      values["prmsl"][index] = p.first; values["2t"][index] = t.first;
      values["10u"][index] = u.first;
      values["10v"][index] = v.first;
      masks["prmsl"][index] = !p.second; masks["2t"][index] = !t.second;
      masks["10u"][index] = masks["10v"][index] = !(u.second && v.second);
    }
    for (const auto& name : {"10u", "10v", "prmsl", "2t"}) {
      const auto missing = std::count_if(masks[name].begin(), masks[name].end(), [](auto value) { return value != 0; });
      if (100.0 * missing / grid.size() > 0.5) throw ValidationError("UKV regridded field has more than 0.5% missing cells");
      output_fields.push_back({hour, name, std::move(values[name]), std::move(masks[name])});
    }
#endif
  }
  const auto cycle_time = ParseUtcDateTime(selected.substr(0, 4) + "-" + selected.substr(4, 2) + "-" +
  ...
  return {"ukmo_ukv", "Met Office UKV 2 km forecast", "uk_deterministic_2km",
          GFSCycle{selected.substr(0, 8), selected.substr(9, 2)}, request.bbox, hours,
          request.output, std::filesystem::file_size(request.output),
          inspection["message_count"].asUInt64(), inspection, urls,
