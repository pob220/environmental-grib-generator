...
public:
  explicit NcFile(const std::filesystem::path& path) {
    ...
    Nc(nc_open(utf8_path.c_str(), NC_NOWRITE, &id_), "opening RTOFS NetCDF");
  }
  ~NcFile() {
...
int Var(int file, const char* name) {
  ...
  Nc(nc_inq_varid(file, name, &id),
     std::string("finding RTOFS variable ") + name);
  return id;
}
...
std::vector<int> Dims(int file, int variable) {
  ...
  Nc(nc_inq_vardimid(file, variable, dims.data()),
     "reading RTOFS dimension ids");
  return dims;
}
...
std::vector<double> ReadAll(int file, int variable) {
  ...
  Nc(nc_get_var_double(file, variable, values.data()),
     "reading RTOFS variable");
  return values;
}
...
std::vector<double> ReadSurface(int file, int variable, std::size_t ny,
  ...
  if (dims.size() < 2)
    throw ValidationError("RTOFS u/v variable has too few dimensions");
...
public:
  explicit DelaunayInterpolator(std::vector<Point> points)
    ...
    if (points_.size() < 3)
      throw ValidationError(
          "RTOFS source subset has fewer than three valid points");
    ...
    const int status =
        qh_new_qhull(qh, 2, static_cast<int>(points_.size()),
    ...
    if (status != qh_ERRnone) {
      ...
      throw ValidationError(
          "Qhull could not triangulate the RTOFS source grid");
    }
    facetT* facet;
    vertexT* vertex;
    vertexT** vertexp;
    ...
    qh_memfreeshort(qh, &current, &maximum);
    if (triangles_.empty())
      throw ValidationError("RTOFS triangulation produced no finite triangles");
    ...
    for (std::size_t index = 0; index < triangles_.size(); ++index) {
      ...
      for (int by = y0; by <= y1; ++by)
        for (int bx = x0; bx <= x1; ++bx)
          bins_[by * kBins + bx].push_back(index);
    }
  }
  ...
  std::pair<double, double> At(double x, double y) const {
    ...
    for (std::size_t triangle_index : candidates) {
      ...
      if (x < triangle.min_x || x > triangle.max_x || y < triangle.min_y ||
          y > triangle.max_y)
        continue;
      const auto& a = points_[triangle.point[0]];
      const auto& b = points_[triangle.point[1]];
      ...
      if (wa >= tolerance && wb >= tolerance && wc >= tolerance)
        return {wa * a.u + wb * b.u + wc * c.u, wa * a.v + wb * b.v + wc * c.v};
    }
...
private:
  ...
  int BinX(double x) const {
    ...
    return std::clamp(
        static_cast<int>((x - min_x_) / (max_x_ - min_x_) * kBins), 0,
        kBins - 1);
  }
  int BinY(double y) const {
    if (max_y_ <= min_y_) return 0;
    return std::clamp(
        static_cast<int>((y - min_y_) / (max_y_ - min_y_) * kBins), 0,
        kBins - 1);
  }
...
CurrentGrid ConvertFile(const std::filesystem::path& path, TimePoint time,
  ...
  if (lat_dims.size() != 2)
    throw ValidationError("RTOFS Latitude must be two-dimensional");
...
std::map<int, std::set<std::string>> ParseInventory(const std::string& html) {
  const std::regex pattern(
      R"(rtofs_glo_3dz_f([0-9]{3})_6hrly_hvr_([A-Za-z_]+)\.nc)");
...
std::vector<RtofsCycle> CycleCandidates(const RtofsRequest& request) {
  if (request.cycle != "auto") {
    if (!request.date)
      throw ValidationError("RTOFS explicit cycle requires a date");
  ...
  for (int day = 0; day < std::max(1, request.max_auto_days); ++day)
    cycles.push_back(
        {FormatUtcDateTime(today - std::chrono::days(day)).substr(0, 10),
         "00"});
  for (auto& cycle : cycles)
    cycle.date.erase(std::remove(cycle.date.begin(), cycle.date.end(), '-'),
                     cycle.date.end());
  return cycles;
}
}  // namespace
...
std::vector<int> RtofsForecastHours(int requested_hours, int step_hours) {
  if (requested_hours <= 0 || step_hours <= 0)
    throw ValidationError("RTOFS hours and step must be positive");
  if (requested_hours > kRtofsMaxForecastHour)
    throw ValidationError("RTOFS hours exceed 192-hour provider limit");
  const int step = std::max(kRtofsNativeStepHours, step_hours);
  std::vector<int> result;
  for (int hour = kRtofsNativeStepHours; hour <= requested_hours; hour += step)
    result.push_back(hour);
  const int rounded = requested_hours - requested_hours % step;
  if (rounded >= kRtofsNativeStepHours &&
      std::find(result.begin(), result.end(), rounded) == result.end())
...
std::string RtofsRegionForBbox(const BoundingBox& bbox) {
  for (const auto& [name, coverage] : kRegions)
    if (coverage.Contains(bbox)) return name;
  throw ValidationError(
      "RTOFS public regional NetCDF files do not cover the requested bbox");
}
std::string RtofsDirectoryUrl(const RtofsCycle& cycle) {
  return std::string(kNomadsBase) + "/" + cycle.DirectoryName() + "/";
}
std::string RtofsFilename(int hour, const std::string& region) {
  char value[128];
  std::snprintf(value, sizeof(value), "rtofs_glo_3dz_f%03d_6hrly_hvr_%s.nc",
                hour, region.c_str());
  return value;
}
std::string RtofsUrl(const RtofsCycle& cycle, int hour,
                     const std::string& region) {
  return RtofsDirectoryUrl(cycle) + RtofsFilename(hour, region);
}
...
RtofsResult GenerateRtofs(const RtofsRequest& request,
  ...
  if (!text_download)
    text_download = [](const std::string& url, double timeout) {
  ...
  for (const auto& candidate : CycleCandidates(request)) {
    try {
      const auto inventory =
          ParseInventory(text_download(RtofsDirectoryUrl(candidate), 30.0));
      bool complete = true;
      for (int hour : hours) {
        auto found = inventory.find(hour);
        if (found == inventory.end() || !found->second.contains(region)) {
          complete = false;
          break;
        }
      }
      ...
      errors.push_back(candidate.DirectoryName() + ": " + error.what());
    }
  }
  if (!selected) throw ValidationError("no complete RTOFS cycle found");
  std::vector<std::filesystem::path> files;
  for (int hour : hours)
    files.push_back(request.download_directory / selected->DirectoryName() /
                    RtofsFilename(hour, region));
  RtofsResult result{request.output,
                     0,
                     0,
                     FormatUtcDateTime(selected->CycleTime()),
                     hours,
                     files,
  ...
  for (std::size_t i = 0; i < files.size(); ++i) {
    if (!std::filesystem::is_regular_file(files[i]) ||
        std::filesystem::file_size(files[i]) == 0) {
      std::filesystem::create_directories(files[i].parent_path());
      downloads.push_back({i, files[i]});
    }
  }
  (void)ParallelMapOrdered(
      downloads, kDefaultDownloadConcurrency,
      [&](const DownloadRequest& download_request) {
        const auto bytes = binary_download(
            RtofsUrl(*selected, hours[download_request.index], region),
            request.timeout_seconds);
        auto temporary = download_request.path;
        temporary += ".tmp";
        try {
          ...
          stream.close();
          std::filesystem::rename(temporary, download_request.path);
        } catch (...) {
          std::error_code ignored;
          std::filesystem::remove(temporary, ignored);
          throw;
        }
        return download_request.index;
      });
  std::lock_guard netcdf_lock(NetcdfApiMutex());
  for (std::size_t i = 0; i < files.size(); ++i) {
    grids.push_back(ConvertFile(
        files[i], selected->CycleTime() + std::chrono::hours(hours[i]),
        request.bbox, grid));
  }
  ...
  result.summary["regridding"] =
      "Qhull Delaunay linear interpolation with nearest fallback";
