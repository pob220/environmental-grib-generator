...
const std::map<std::string, std::string> kRoutingFields{
    ...
    {"var_TMP", "on"}, {"lev_10_m_above_ground", "on"},
    {"lev_mean_sea_level", "on"}, {"lev_2_m_above_ground", "on"}};
const std::map<std::string, std::string> kMinimalFields{
    {"var_UGRD", "on"}, {"var_VGRD", "on"},
    {"lev_10_m_above_ground", "on"}};
const std::map<std::string, std::string> kMarineFields{
    {"var_GUST", "on"}, {"var_TCDC", "on"}, {"var_APCP", "on"},
    {"lev_surface", "on"}, {"lev_entire_atmosphere", "on"}};
const std::map<std::string, std::string> kWaveFields{
...
const std::map<std::string, std::string> kDwdRoutingFields{
    ...
    {"2t", "t_2m"}};
const std::map<std::string, std::string> kDwdMinimalFields{
    {"10u", "u_10m"}, {"10v", "v_10m"}};
...
std::string Encode(const std::string& value) {
  ...
  if (!encoded) { curl_easy_cleanup(curl); throw Error("URL encoding failed"); }
...
std::string Query(const std::vector<std::pair<std::string, std::string>>& values) {
...
void ValidateDownloaded(const std::vector<unsigned char>& bytes,
  ...
  if (bytes.empty()) throw ValidationError(provider + " download returned empty response");
  auto first = std::find_if_not(bytes.begin(), bytes.end(), [](unsigned char value) {
    return value == ' ' || value == '\r' || value == '\n' || value == '\t';
  });
  if (first != bytes.end() && *first == '<') {
    throw ValidationError(provider + " download returned HTML/text instead of GRIB2");
  }
  const std::size_t prefix = std::min<std::size_t>(32, bytes.size());
  static constexpr std::array<unsigned char, 4> kMarker{'G', 'R', 'I', 'B'};
  if (std::search(bytes.begin(), bytes.begin() + static_cast<std::ptrdiff_t>(prefix),
                  kMarker.begin(), kMarker.end()) ==
      bytes.begin() + static_cast<std::ptrdiff_t>(prefix)) {
    throw ValidationError(provider + " download did not start with a GRIB message");
  }
...
std::vector<GFSCycle> SixHourlyCycles(const GFSRequest& request,
  ...
  if (request.cycle != "auto") {
    ...
    if (!valid.contains(request.cycle) || !request.date) throw ValidationError("explicit cycle requires date and 00, 06, 12, or 18");
    ValidateDate(*request.date);
    return {{*request.date, request.cycle}};
  }
  TimePoint value = now.value_or(std::chrono::floor<std::chrono::seconds>(std::chrono::system_clock::now()));
  std::time_t raw = static_cast<std::time_t>(value.time_since_epoch().count());
  std::tm tm{}; gmtime_r(&raw, &tm);
  tm.tm_hour = (tm.tm_hour / 6) * 6; tm.tm_min = 0; tm.tm_sec = 0;
  TimePoint cursor{std::chrono::seconds{timegm(&tm)}};
  std::vector<GFSCycle> result;
  for (int i = 0; i < std::max(1, request.max_auto_cycles); ++i) {
    raw = static_cast<std::time_t>(cursor.time_since_epoch().count()); gmtime_r(&raw, &tm);
    std::ostringstream date, cycle; date << std::put_time(&tm, "%Y%m%d"); cycle << std::put_time(&tm, "%H");
    result.push_back({date.str(), cycle.str()}); cursor -= std::chrono::hours{6};
  }
  return result;
}
...
std::vector<unsigned char> DecompressBzip2(
  ...
  while (capacity <= kLimit) {
    ...
    if (status != BZ_OUTBUFF_FULL) {
      throw ValidationError("DWD ICON-EU compressed GRIB could not be decompressed");
    }
    capacity *= 2;
  }
  throw ValidationError("DWD ICON-EU decompressed field exceeds 1 GiB safety limit");
}

Json::Value BboxJson(const BoundingBox& bbox) {
  Json::Value value(Json::objectValue);
  value["west"] = bbox.west; value["south"] = bbox.south;
  value["east"] = bbox.east; value["north"] = bbox.north;
  return value;
}
...
std::vector<WeatherProvider> ListWeatherProviders() {
  return {
      {"gfs", "NOAA GFS 0.25 degree global forecast", "NOAA NOMADS", "GRIB2",
       "free/no account", "Global Forecast System subsets from the official NOMADS filter."},
      ...
      {"copernicus_global_waves", "Copernicus Marine Global Waves forecast",
       ...
       "Copernicus Marine account required", "Global Ocean Waves Analysis and Forecast product via native ARCO chunk decoding.", true},
      {"noaa_hrrr", "NOAA HRRR 3 km forecast", "NOAA NOMADS", "GRIB2",
       "free/no account", "Indexed full-grid GRIB messages; bbox cropping is not yet available."},
      {"ukmo_ukv", "Met Office UKV 2 km forecast", "Met Office AWS/Open Data",
       "NetCDF source, converted to OpenCPN GRIB", "free/no account",
       UkvProjectionAvailable() ? "Native projection-aware NetCDF regridding."
      ...
      {"dwd_icon_eu", "DWD ICON-EU 13 km forecast", "DWD Open Data", "GRIB2",
       "free/no account", "Full-domain DWD regular-lat/lon field files; bbox cropping is not yet available."},
      {"ecmwf_ifs_open", "ECMWF IFS Open Data forecast", "ECMWF Open Data", "GRIB2",
       "free/no account", "Indexed global Open Data surface fields; bbox cropping is not yet available."},
...
std::vector<GFSCycle> GfsCycleCandidates(const GFSRequest& request,
                                         std::optional<TimePoint> now) {
  if (request.cycle != "auto") {
    static const std::set<std::string> valid{"00", "03", "06", "09", "12", "15", "18", "21"};
    if (!valid.contains(request.cycle)) throw ValidationError("invalid explicit GFS cycle");
    if (!request.date) throw ValidationError("date YYYYMMDD is required when cycle is explicit");
    ValidateDate(*request.date);
    return {{*request.date, request.cycle}};
  }
  TimePoint value = now.value_or(std::chrono::floor<std::chrono::seconds>(std::chrono::system_clock::now()));
  ...
  for (int i = 0; i < std::max(1, request.max_auto_cycles); ++i) {
    ...
    date << std::put_time(&tm, "%Y%m%d"); cycle << std::put_time(&tm, "%H");
...
std::string BuildGfsFilterUrl(const GFSCycle& cycle, int forecast_hour,
  ...
  std::vector<std::pair<std::string, std::string>> query{
      {"dir", cycle.Directory()},
      {"file", "gfs.t" + cycle.cycle + "z.pgrb2.0p25.f" + FormatHour(forecast_hour)},
      {"subregion", ""}, {"leftlon", FormatNumber(bbox.west)},
      {"rightlon", FormatNumber(bbox.east)}, {"toplat", FormatNumber(bbox.north)},
...
std::string BuildGfsWaveFilterUrl(const GFSCycle& cycle, int forecast_hour,
                                  const BoundingBox& bbox) {
  std::ostringstream hour; hour << std::setw(3) << std::setfill('0') << forecast_hour;
  std::vector<std::pair<std::string, std::string>> query{
      {"dir", "/gfs." + cycle.date + "/" + cycle.cycle + "/wave/gridded"},
      {"file", "gfswave.t" + cycle.cycle + "z.global.0p25.f" + hour.str() + ".grib2"},
      {"subregion", ""}, {"leftlon", FormatNumber(bbox.west)},
      {"rightlon", FormatNumber(bbox.east)}, {"toplat", FormatNumber(bbox.north)},
      {"bottomlat", FormatNumber(bbox.south)}};
...
std::vector<unsigned char> CurlHttpGet(const std::string& url,
  ...
  curl_easy_setopt(curl, CURLOPT_TIMEOUT_MS, static_cast<long>(timeout_seconds * 1000.0));
  curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT_MS, static_cast<long>(std::min(timeout_seconds, 20.0) * 1000.0));
  curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, CurlWrite);
  curl_easy_setopt(curl, CURLOPT_WRITEDATA, &output);
  curl_easy_setopt(curl, CURLOPT_MAXFILESIZE_LARGE, static_cast<curl_off_t>(512ULL * 1024ULL * 1024ULL));
...
WeatherGenerateResult GenerateGfs(const GFSRequest& request, HttpGet http_get,
  ...
  if (!std::set<int>{1, 3, 6, 12}.contains(request.step_hours)) throw ValidationError("step-hours must be one of 1, 3, 6, or 12 for GFS");
  const auto hours = ForecastHourSequence(request.hours, request.step_hours);
  const auto fields = request.waves ? kWaveFields : GfsVariablesForPreset(request.preset);
  ...
  if (request.dry_run) {
    WeatherGenerateResult result{request.waves ? "gfs_wave" : "gfs",
        request.waves ? "NOAA GFS Wave forecast via NOMADS" : "NOAA GFS 0.25 degree forecast via NOMADS",
        request.waves ? "gfswave_global_0p25" : "gfs_0p25", cycles.front(), request.bbox,
        hours, request.output, 0, 0, Json::Value(Json::objectValue), {}, fields};
    result.inspection["stream_valid"] = false; result.inspection["message_count"] = 0; result.inspection["dry_run"] = true;
  ...
  for (const auto& cycle : cycles) {
    try {
      segments.clear(); urls.clear();
      for (int hour : hours) {
        const std::string url = url_for(cycle, hour);
        Json::Value details; details["cycle"] = cycle.CycleTime(); details["hour"] = hour;
        ...
        urls.push_back(url); segments.push_back(std::move(bytes));
      }
      selected = cycle;
      break;
  ...
  if (!selected) {
    std::ostringstream message; message << "No complete GFS cycle was available. Tried: ";
    for (const auto& error : errors) message << error << "; ";
    throw ValidationError(message.str());
  }
  std::filesystem::create_directories(request.output.parent_path().empty() ? "." : request.output.parent_path());
  const auto temporary = TemporarySibling(request.output);
  try {
    std::ofstream output(temporary, std::ios::binary | std::ios::trunc);
    for (const auto& segment : segments) output.write(reinterpret_cast<const char*>(segment.data()), static_cast<std::streamsize>(segment.size()));
    ...
    return {request.waves ? "gfs_wave" : "gfs",
            request.waves ? "NOAA GFS Wave forecast via NOMADS" : "NOAA GFS 0.25 degree forecast via NOMADS",
            request.waves ? "gfswave_global_0p25" : "gfs_0p25", *selected,
            request.bbox, hours, request.output, scan.byte_count, scan.message_count,
            inspection, urls, fields};
  } catch (...) {
...
std::vector<int> DwdIconEuForecastHourSequence(int hours, int step_hours) {
  if (hours > 120) throw ValidationError("DWD ICON-EU forecasts are supported only to 120 hours");
  if (step_hours != 1 && step_hours != 3) throw ValidationError("step-hours must be 1 or 3 for DWD ICON-EU");
...
std::vector<GFSCycle> DwdIconEuCycleCandidates(
    const GFSRequest& request, std::optional<TimePoint> now) {
  if (request.cycle != "auto") {
    static const std::set<std::string> valid{"00", "03", "06", "09", "12", "15", "18", "21"};
    if (!valid.contains(request.cycle) || !request.date) throw ValidationError("explicit ICON-EU cycle requires valid cycle and date");
    ValidateDate(*request.date);
    return {{*request.date, request.cycle}};
  }
  TimePoint value = now.value_or(std::chrono::floor<std::chrono::seconds>(std::chrono::system_clock::now()));
  std::time_t raw = static_cast<std::time_t>(value.time_since_epoch().count());
  std::tm tm{}; gmtime_r(&raw, &tm);
  tm.tm_hour = (tm.tm_hour / 3) * 3; tm.tm_min = 0; tm.tm_sec = 0;
  TimePoint cursor{std::chrono::seconds{timegm(&tm)}};
  std::vector<GFSCycle> result;
  for (int i = 0; i < std::max(1, request.max_auto_cycles); ++i) {
    raw = static_cast<std::time_t>(cursor.time_since_epoch().count()); gmtime_r(&raw, &tm);
    std::ostringstream date, cycle; date << std::put_time(&tm, "%Y%m%d"); cycle << std::put_time(&tm, "%H");
    result.push_back({date.str(), cycle.str()}); cursor -= std::chrono::hours{3};
  }
  return result;
}
...
WeatherGenerateResult GenerateDwdIconEu(
  ...
  if (!domain.Contains(request.bbox)) throw ValidationError("DWD ICON-EU bbox is outside the supported Europe regional domain");
  const auto hours = DwdIconEuForecastHourSequence(request.hours, request.step_hours);
  const auto fields = DwdIconEuFieldsForPreset(request.preset);
  const auto cycles = DwdIconEuCycleCandidates(request, now);
  if (request.dry_run) {
    WeatherGenerateResult result{"dwd_icon_eu", "DWD ICON-EU 13 km forecast via Open Data",
      "icon_eu_regular_lat_lon_13km", cycles.front(), request.bbox, hours,
      request.output, 0, 0, Json::Value(Json::objectValue), {}, fields};
    result.inspection["dry_run"] = true; result.inspection["stream_valid"] = false;
    for (int hour : hours) for (const auto& [short_name, field] : fields) {
      (void)short_name; result.urls.push_back(BuildDwdIconEuUrl(cycles.front(), hour, field));
    }
    return result;
  }
  if (!http_get) http_get = CurlHttpGet;
  if (std::filesystem::exists(request.output) && !request.overwrite) throw ValidationError("output already exists; enable overwrite");
  std::vector<std::vector<unsigned char>> segments;
  ...
  for (const auto& cycle : cycles) {
    try {
      ...
      for (int hour : hours) for (const auto& [short_name, field] : fields) {
        ...
        Json::Value detail; detail["cycle"] = cycle.CycleTime(); detail["hour"] = hour; detail["field"] = field;
        ...
        urls.push_back(url); segments.push_back(std::move(data));
      }
      selected = cycle; break;
    } catch (const ValidationError&) {
      if (request.cycle != "auto") throw;
    }
  }
  if (!selected) throw ValidationError("No complete DWD ICON-EU cycle was available");
  std::filesystem::create_directories(request.output.parent_path().empty() ? "." : request.output.parent_path());
  const auto temporary = TemporarySibling(request.output);
  try {
    std::ofstream output(temporary, std::ios::binary | std::ios::trunc);
    ...
    const auto inspection = InspectGrib(temporary);
    std::filesystem::rename(temporary, request.output);
    return {"dwd_icon_eu", "DWD ICON-EU 13 km forecast via Open Data",
            "icon_eu_regular_lat_lon_13km", *selected, request.bbox, hours,
            request.output, scan.byte_count, scan.message_count, inspection,
            urls, fields};
  } catch (...) {
    std::error_code ignored; std::filesystem::remove(temporary, ignored); throw;
  }
...
std::vector<HrrrInventoryEntry> ParseHrrrInventory(const std::string& text) {
  ...
  while (std::getline(lines, line)) {
    ...
    try {
      ...
      if (used_number != parts[0].size() || used_offset != parts[1].size()) continue;
      result.push_back({number, static_cast<std::size_t>(offset), parts[3], parts[4], parts[5]});
    } catch (...) {
    }
  }
  return result;
...
std::vector<int> HrrrForecastHourSequence(int hours, int step_hours) {
  if (hours > 48) throw ValidationError("HRRR forecasts are supported only to 48 hours");
...
WeatherGenerateResult GenerateHrrr(
    const GFSRequest& request, HttpGet http_get,
    HttpGetRange http_get_range, std::optional<TimePoint> now,
    ProgressCallback progress) {
  request.bbox.Validate();
  if (!BoundingBox{-130.0, 20.0, -60.0, 55.0}.Contains(request.bbox)) throw ValidationError("HRRR bbox is outside the supported contiguous United States domain");
  const auto hours = HrrrForecastHourSequence(request.hours, request.step_hours);
  const auto fields = HrrrVariablesForPreset(request.preset);
  std::vector<GFSCycle> cycles;
  if (request.cycle != "auto") {
    if (!request.date || request.cycle.size() != 2 || std::stoi(request.cycle) > 23) throw ValidationError("explicit HRRR cycle requires date and hour 00-23");
    ValidateDate(*request.date); cycles.push_back({*request.date, request.cycle});
  } else {
    TimePoint value = now.value_or(std::chrono::floor<std::chrono::seconds>(std::chrono::system_clock::now())) - std::chrono::hours{1};
    for (int i = 0; i < std::max(1, request.max_auto_cycles); ++i) {
      const std::time_t raw = static_cast<std::time_t>(value.time_since_epoch().count());
      std::tm tm{}; gmtime_r(&raw, &tm);
      std::ostringstream date, cycle; date << std::put_time(&tm, "%Y%m%d"); cycle << std::put_time(&tm, "%H");
      cycles.push_back({date.str(), cycle.str()}); value -= std::chrono::hours{1};
    }
  }
  if (request.dry_run) {
    WeatherGenerateResult result{"noaa_hrrr", "NOAA HRRR 3 km forecast via NOMADS",
      "hrrr_conus_3km", cycles.front(), request.bbox, hours, request.output,
      0, 0, Json::Value(Json::objectValue), {}, fields};
    result.inspection["dry_run"] = true;
    for (int hour : hours) result.urls.push_back(BuildHrrrIndexUrl(cycles.front(), hour));
    return result;
  }
  if (!http_get) http_get = CurlHttpGet;
  ...
  std::vector<std::string> urls;
  const std::map<std::string, std::string> level_for{{"UGRD", "10 m above ground"},
      {"VGRD", "10 m above ground"}, {"TMP", "2 m above ground"},
      {"PRES", "surface"}, {"GUST", "surface"}};
  std::set<std::pair<std::string, std::string>> requested;
  for (const auto& [name, level] : level_for) {
    if (fields.contains("var_" + name) && fields.contains("lev_" + std::string(level == "10 m above ground" ? "10_m_above_ground" : level == "2 m above ground" ? "2_m_above_ground" : "surface"))) requested.insert({name, level});
  }
  for (const auto& cycle : cycles) {
    try {
      hour_segments.clear(); urls.clear();
      for (int hour : hours) {
        const auto index_url = BuildHrrrIndexUrl(cycle, hour);
        const auto file_url = BuildHrrrFileUrl(cycle, hour);
        Json::Value detail; detail["cycle"] = cycle.CycleTime(); detail["hour"] = hour;
        ...
        if (inventory.empty()) throw ValidationError("HRRR inventory was empty or unreadable");
        std::vector<unsigned char> combined;
        std::set<std::pair<std::string, std::string>> found;
        for (std::size_t i = 0; i < inventory.size(); ++i) {
          const auto key = std::make_pair(inventory[i].short_name, inventory[i].level);
          if (!requested.contains(key)) continue;
          if (i + 1 == inventory.size()) throw ValidationError("HRRR selected final inventory message without following offset");
        ...
        if (found != requested) throw ValidationError("HRRR inventory did not contain all required fields");
        hour_segments.push_back(std::move(combined)); urls.push_back(file_url);
      }
      selected = cycle; break;
    } catch (const ValidationError&) {
      if (request.cycle != "auto") throw;
  ...
  try {
    std::ofstream output(temporary, std::ios::binary | std::ios::trunc);
    for (const auto& segment : hour_segments) output.write(reinterpret_cast<const char*>(segment.data()), static_cast<std::streamsize>(segment.size()));
    output.close();
    const auto scan = ScanGribMessages(temporary);
    const auto inspection = InspectGrib(temporary);
    std::filesystem::rename(temporary, request.output);
    return {"noaa_hrrr", "NOAA HRRR 3 km forecast via NOMADS", "hrrr_conus_3km",
            *selected, request.bbox, hours, request.output, scan.byte_count,
            scan.message_count, inspection, urls, fields};
  } catch (...) {
    std::error_code ignored; std::filesystem::remove(temporary, ignored); throw;
  }
}
...
std::string BuildEcmwfDataUrl(const GFSCycle& cycle, int forecast_hour,
  ...
  return "https://data.ecmwf.int/forecasts/" + cycle.date + "/" + cycle.cycle +
         "z/" + model + "/0p25/oper/" + cycle.date + cycle.cycle +
         "0000-" + std::to_string(forecast_hour) + "h-oper-fc.grib2";
}
...
WeatherGenerateResult GenerateEcmwfOpenData(
    ...
    ProgressCallback progress) {
  request.bbox.Validate();
  if (aifs && request.step_hours != 6 && request.step_hours != 12) throw ValidationError("step-hours must be 6 or 12 for ECMWF AIFS Open Data");
  const auto hours = ForecastHourSequence(request.hours, request.step_hours);
  const auto cycles = SixHourlyCycles(request, now);
  const std::map<std::string, std::string> fields{{"10u", "on"}, {"10v", "on"}, {"msl", "on"}, {"2t", "on"}};
  if (request.dry_run) {
    WeatherGenerateResult result{aifs ? "ecmwf_aifs_open" : "ecmwf_ifs_open",
      aifs ? "ECMWF AIFS Open Data forecast" : "ECMWF IFS Open Data forecast",
      aifs ? "ecmwf_aifs_open" : "ecmwf_ifs_open_0p25", cycles.front(),
      request.bbox, hours, request.output, 0, 0, Json::Value(Json::objectValue), {}, fields};
    result.inspection["dry_run"] = true;
    for (int hour : hours) result.urls.push_back(BuildEcmwfDataUrl(cycles.front(), hour, aifs));
    return result;
  }
  if (!http_get) http_get = CurlHttpGet;
  if (!http_get_range) http_get_range = CurlHttpGetRange;
  ...
  for (const auto& cycle : cycles) {
    try {
      ...
      for (int hour : hours) {
        ...
        const auto index_bytes = http_get(BuildEcmwfIndexUrl(cycle, hour, aifs), request.timeout_seconds);
        std::istringstream lines(std::string(index_bytes.begin(), index_bytes.end()));
        std::string line;
        ...
        while (std::getline(lines, line)) {
          ...
          if (!wanted.contains(parameter) || type != "fc" || level != "sfc" ||
              step != std::to_string(hour)) continue;
          const auto offset = entry["_offset"].asUInt64();
          const auto length = entry["_length"].asUInt64();
          if (length == 0) throw ValidationError("ECMWF index contains zero-length selected field");
          ...
          Json::Value detail; detail["cycle"] = cycle.CycleTime(); detail["hour"] = hour; detail["field"] = parameter;
          Progress(progress, "downloaded ECMWF indexed field", detail);
        }
        if (found != wanted) throw ValidationError("ECMWF index did not contain all requested surface fields");
        segments.push_back(std::move(combined)); urls.push_back(data_url);
      }
      selected = cycle; break;
    } catch (const ValidationError&) {
      if (request.cycle != "auto") throw;
  ...
  if (!selected) throw ValidationError("No complete ECMWF Open Data cycle was available");
  std::filesystem::create_directories(request.output.parent_path().empty() ? "." : request.output.parent_path());
  const auto temporary = TemporarySibling(request.output);
  try {
    std::ofstream output(temporary, std::ios::binary | std::ios::trunc);
    ...
    const auto inspection = InspectGrib(temporary);
    std::filesystem::rename(temporary, request.output);
    return {aifs ? "ecmwf_aifs_open" : "ecmwf_ifs_open",
            aifs ? "ECMWF AIFS Open Data forecast" : "ECMWF IFS Open Data forecast",
            aifs ? "ecmwf_aifs_open" : "ecmwf_ifs_open_0p25", *selected,
            request.bbox, hours, request.output, scan.byte_count, scan.message_count,
            inspection, urls, fields};
  } catch (...) {
    std::error_code ignored; std::filesystem::remove(temporary, ignored); throw;
...
Json::Value WeatherResultJson(const WeatherGenerateResult& result) {
  Json::Value value(Json::objectValue);
  value["provider"] = result.provider; value["source"] = result.source;
  value["model"] = result.model; value["cycle"] = result.cycle.CycleTime();
  value["bbox"] = BboxJson(result.bbox); value["output"] = result.output.string();
  ...
  for (const auto& [key, entry] : result.variables_levels) value["variables_levels"][key] = entry;
