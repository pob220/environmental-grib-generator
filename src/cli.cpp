...
std::string RequireValue(const std::vector<std::string>& args, std::size_t& index,
  ...
  if (++index >= args.size()) throw eg::ValidationError(option + " requires a value");
  return args[index];
}
...
Json::Value ProviderJson(const eg::Provider& provider) {
  ...
  for (const auto& variable : provider.variables) value["variables"].append(variable);
  return value;
}
...
int Providers() {
  ...
  for (const auto& provider : registry.List()) result.append(ProviderJson(provider));
  PrintJson(result);
  return 0;
}
...
int WeatherProviders() {
  ...
  for (const auto& provider : eg::ListWeatherProviders()) {
    Json::Value value(Json::objectValue);
    value["id"] = provider.id; value["label"] = provider.label;
    value["source"] = provider.source; value["format"] = provider.format;
    value["account"] = provider.account; value["description"] = provider.description;
    value["implemented"] = provider.implemented;
    result.append(value);
  }
  PrintJson(result);
...
int GenerateWeather(const std::vector<std::string>& args) {
  ...
  std::string username, password_environment = "CURRENTGRIB_COPERNICUS_PASSWORD";
  std::optional<double> grid_spacing;
  for (std::size_t i = 1; i < args.size(); ++i) {
    const auto& arg = args[i];
    if (arg == "--provider") provider = RequireValue(args, i, arg);
    else if (arg == "--bbox") {
      if (i + 4 >= args.size()) throw eg::ValidationError("--bbox requires W S E N");
      request.bbox = {ParseDouble(args[++i], arg), ParseDouble(args[++i], arg),
                      ParseDouble(args[++i], arg), ParseDouble(args[++i], arg)};
      have_bbox = true;
    } else if (arg == "--hours") request.hours = ParseInt(RequireValue(args, i, arg), arg);
    else if (arg == "--start") start = eg::ParseUtcDateTime(RequireValue(args, i, arg));
    else if (arg == "--step-hours") request.step_hours = ParseInt(RequireValue(args, i, arg), arg);
    else if (arg == "--cycle") request.cycle = RequireValue(args, i, arg);
    else if (arg == "--date") request.date = RequireValue(args, i, arg);
    else if (arg == "--weather-preset") request.preset = RequireValue(args, i, arg);
    else if (arg == "--username") username = RequireValue(args, i, arg);
    else if (arg == "--password-env") password_environment = RequireValue(args, i, arg);
    else if (arg == "--weather-grid-spacing-deg" || arg == "--grid-spacing-deg") grid_spacing = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--output") request.output = RequireValue(args, i, arg);
    else if (arg == "--overwrite") request.overwrite = true;
    else if (arg == "--dry-run") request.dry_run = true;
    else if (arg == "--timeout-seconds") request.timeout_seconds = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--json" || arg == "--metadata-summary") {}
    else throw eg::ValidationError("unknown generate-weather option: " + arg);
  }
  if (!have_bbox || request.output.empty()) throw eg::ValidationError("generate-weather requires --bbox and --output");
  if (provider == "gfs") {
    request.waves = false;
    ...
    if (!start) throw eg::ValidationError("Copernicus Global Waves requires --start");
    std::string password;
    if (const char* value = std::getenv(password_environment.c_str())) password = value;
    ...
    throw eg::UnsupportedSourceError("native weather provider is not yet available: " + provider);
  }
  return 0;
}
...
int GenerateProvider(const std::vector<std::string>& args) {
  ...
  for (std::size_t i = 1; i < args.size(); ++i) {
    if (args[i] == "--provider") provider = RequireValue(args, i, args[i]);
    else if (args[i] == "--output") output = RequireValue(args, i, args[i]);
    else if (args[i] == "--overwrite") overwrite = true;
    else if (args[i] == "--dry-run") dry_run = true;
    else if (args[i] == "--json" || args[i] == "--metadata-summary" || args[i] == "--verbose") {}
    else throw eg::ValidationError("unknown generate-provider option: " + args[i]);
  }
  if (provider.empty() || output.empty()) throw eg::ValidationError("generate-provider requires --provider and --output");
  if (provider != "marine_ie_irish_sea") throw eg::UnsupportedSourceError("native current provider is not yet available: " + provider);
  if (dry_run) {
    Json::Value result; result["provider"] = provider; result["output"] = output.string(); result["dry_run"] = true;
    PrintJson(result); return 0;
  }
  PrintJson(eg::DirectCurrentResultJson(eg::DownloadMarineIe(output, overwrite)));
  return 0;
}
...
int GenerateCopernicus(const std::vector<std::string>& args) {
  ...
  for (std::size_t i = 1; i < args.size(); ++i) {
    const auto& arg = args[i];
    if (arg == "--provider") request.provider = RequireValue(args, i, arg);
    else if (arg == "--bbox") {
      if (i + 4 >= args.size()) throw eg::ValidationError("--bbox requires W S E N");
      request.bbox = {ParseDouble(args[++i], arg), ParseDouble(args[++i], arg),
                      ParseDouble(args[++i], arg), ParseDouble(args[++i], arg)};
      have_bbox = true;
    } else if (arg == "--start") { request.start = eg::ParseUtcDateTime(RequireValue(args, i, arg)); have_start = true; }
    else if (arg == "--hours") request.hours = ParseInt(RequireValue(args, i, arg), arg);
    else if (arg == "--step-hours") request.step_hours = ParseInt(RequireValue(args, i, arg), arg);
    else if (arg == "--grid-spacing-deg") request.grid_spacing_deg = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--username") request.username = RequireValue(args, i, arg);
    else if (arg == "--password-env") password_environment = RequireValue(args, i, arg);
    else if (arg == "--output") request.output = RequireValue(args, i, arg);
    else if (arg == "--overwrite") request.overwrite = true;
    else if (arg == "--dry-run") request.dry_run = true;
    else if (arg == "--timeout-seconds") request.timeout_seconds = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--json" || arg == "--metadata-summary" || arg == "--verbose") {}
    else throw eg::ValidationError("unknown generate-copernicus option: " + arg);
  }
  if (const char* password = std::getenv(password_environment.c_str())) request.password = password;
  if (!have_bbox || !have_start || request.output.empty())
    throw eg::ValidationError("generate-copernicus requires --bbox, --start, and --output");
  PrintJson(eg::CopernicusResultJson(
      request.provider == "copernicus_global"
...
int GenerateRtofs(const std::vector<std::string>& args) {
  ...
  for (std::size_t i = 1; i < args.size(); ++i) {
    ...
    else if (arg == "--download-directory") request.download_directory = RequireValue(args, i, arg);
    else if (arg == "--grid-spacing-deg") request.grid_spacing_deg = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--output") request.output = RequireValue(args, i, arg);
    else if (arg == "--overwrite") request.overwrite = true;
    else if (arg == "--dry-run") request.dry_run = true;
    ...
    else throw eg::ValidationError("unknown generate-rtofs option: " + arg);
  }
  if (!have_bbox || request.output.empty() || request.download_directory.empty())
    throw eg::ValidationError("generate-rtofs requires --bbox, --download-directory, and --output");
...
int GenerateEnvironment(const std::vector<std::string>& args) {
  ...
  for (std::size_t i = 1; i < args.size(); ++i) {
    ...
    if (arg == "--bbox") {
    ...
    else if (arg == "--weather-provider") request.weather_provider = RequireValue(args, i, arg);
    else if (arg == "--weather-preset") request.weather_preset = RequireValue(args, i, arg);
    else if (arg == "--weather-file") request.weather_file = RequireValue(args, i, arg);
    else if (arg == "--include-waves") request.include_waves = true;
    else if (arg == "--wave-provider") request.wave_provider = RequireValue(args, i, arg);
    else if (arg == "--wave-step-hours") request.wave_step_hours = ParseInt(RequireValue(args, i, arg), arg);
    else if (arg == "--current-source") request.current_source = RequireValue(args, i, arg);
      ...
      if (const char* password = std::getenv(name.c_str())) request.copernicus_password = password;
    }
    else if (arg == "--download-directory") request.download_directory = RequireValue(args, i, arg);
    else if (arg == "--grid-spacing-deg") request.current_grid_spacing_deg = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--output") request.output = RequireValue(args, i, arg);
    else if (arg == "--overwrite") request.overwrite = true;
    else if (arg == "--keep-intermediate") request.keep_intermediate = true;
    else if (arg == "--dry-run") request.dry_run = true;
    else if (arg == "--weather-grid-spacing-deg") request.weather_grid_spacing_deg = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--metadata-summary" || arg == "--verbose" || arg == "--json") {}
    else throw eg::ValidationError("unknown generate-environment-grib option: " + arg);
  }
  if (!have_bbox || !have_start || request.output.empty()) throw eg::ValidationError("generate-environment-grib requires --bbox, --start, and --output");
  auto progress = [](const std::string& stage, const Json::Value& details) {
    std::cerr << stage;
    if (details.isMember("detail")) std::cerr << ": " << details["detail"].asString();
    std::cerr << '\n';
  };
  PrintJson(eg::EnvironmentResultJson(eg::GenerateEnvironment(request, {}, std::nullopt, progress)));
  return 0;
}
...
int Normalize(const std::vector<std::string>& args) {
  if (args.size() != 3) throw eg::ValidationError("usage: environmental-grib normalize-grib INPUT OUTPUT");
...
int Generate(const std::vector<std::string>& args) {
  ...
  for (std::size_t i = 1; i < args.size(); ++i) {
    ...
    if (arg == "--bbox") {
      if (i + 4 >= args.size()) throw eg::ValidationError("--bbox requires W S E N");
      bbox = {ParseDouble(args[++i], "--bbox"), ParseDouble(args[++i], "--bbox"),
              ParseDouble(args[++i], "--bbox"), ParseDouble(args[++i], "--bbox")};
      have_bbox = true;
    } else if (arg == "--start") start = RequireValue(args, i, arg);
    else if (arg == "--hours") hours = ParseInt(RequireValue(args, i, arg), arg);
    else if (arg == "--step-hours") step_hours = ParseInt(RequireValue(args, i, arg), arg);
    else if (arg == "--grid-spacing-deg") spacing = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--source") source = RequireValue(args, i, arg);
    else if (arg == "--input-netcdf") input_netcdf = RequireValue(args, i, arg);
    else if (arg == "--input-cache") input_cache = RequireValue(args, i, arg);
    else if (arg == "--model-dir" || arg == "--model-directory") model_directory = RequireValue(args, i, arg);
    else if (arg == "--no-infer-minor") infer_minor = false;
    else if (arg == "--overwrite") overwrite = true;
    else if (arg == "--u-variable") netcdf_options.u_variable = RequireValue(args, i, arg);
    else if (arg == "--v-variable") netcdf_options.v_variable = RequireValue(args, i, arg);
    else if (arg == "--lat-variable") netcdf_options.lat_variable = RequireValue(args, i, arg);
    else if (arg == "--lon-variable") netcdf_options.lon_variable = RequireValue(args, i, arg);
    else if (arg == "--time-variable") netcdf_options.time_variable = RequireValue(args, i, arg);
    else if (arg == "--depth-index") netcdf_options.depth_index = ParseInt(RequireValue(args, i, arg), arg);
    else if (arg == "--depth-value") netcdf_options.depth_value = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--assume-units") netcdf_options.assume_units = RequireValue(args, i, arg);
    else if (arg == "--nearest-time") netcdf_options.nearest_time = true;
    else if (arg == "--use-source-grid") netcdf_options.use_source_grid = true;
    else if (arg == "--coverage-tolerance-deg") netcdf_options.coverage_tolerance_deg = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--source-grid-regularity-tolerance") netcdf_options.source_grid_regularity_tolerance = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--u") u = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--v") v = ParseDouble(RequireValue(args, i, arg), arg);
    else if (arg == "--units") units = RequireValue(args, i, arg);
    else if (arg == "--output") output = RequireValue(args, i, arg);
    else if (arg == "--json" || arg == "--metadata-summary") {}
    else throw eg::ValidationError("unknown generate option: " + arg);
  }
  if (source == "tpxo-cache") {
    if (start.empty() || hours < 0 || output.empty() || input_cache.empty())
      throw eg::ValidationError("tpxo-cache generation requires --input-cache, --start, --hours, and --output");
  ...
  if (source == "tpxo") {
    if (!have_bbox || start.empty() || hours < 0 || output.empty() || model_directory.empty())
      throw eg::ValidationError("tpxo generation requires --model-dir, --bbox, --start, --hours, and --output");
    eg::TpxoModelRequest request{bbox,eg::ParseUtcDateTime(start),hours,
                                 step_hours,spacing,model_directory,output,
                                 infer_minor,overwrite};
    const auto generated=eg::GenerateFromTpxo10AtlasModel(request);
    Json::Value result(Json::objectValue); result["source"]=source;
    result["output"]=generated.output.string(); result["message_count"]=Json::UInt64(generated.message_count);
    result["byte_count"]=Json::UInt64(generated.byte_count); result["inspection"]=generated.inspection;
    PrintJson(result); return 0;
  }
  if (!have_bbox || start.empty() || hours < 0 || output.empty()) {
    throw eg::ValidationError("generate requires --bbox, --start, --hours, and --output");
  }
  bbox.Validate();
  std::optional<eg::NetCDFCurrentSource> netcdf;
  if (source == "netcdf") {
    if (input_netcdf.empty()) throw eg::ValidationError("netcdf source requires --input-netcdf");
  ...
  const auto times = eg::BuildTimeSequence(eg::ParseUtcDateTime(start), hours, step_hours);
  std::vector<eg::CurrentGrid> fields;
  fields.reserve(times.size());
  for (const auto time : times) {
    if (source == "synthetic") fields.push_back(eg::MakeSyntheticRotaryCurrent(bbox, time, grid));
    else if (source == "constant") fields.push_back(eg::MakeConstantCurrent(bbox, time, grid, u, v, units));
    else if (source == "netcdf") fields.push_back(netcdf->GetCurrentGrid(bbox, time, grid));
    else throw eg::UnsupportedSourceError("native provider is not yet available: " + source);
  }
...
int InspectTpxo(const std::vector<std::string>& args) {
  if (args.size() != 2)
    throw eg::ValidationError("usage: environmental-grib inspect-tpxo-cache FILE");
...
int PrepareTpxo(const std::vector<std::string>& args) {
  ...
  for(std::size_t i=1;i<args.size();++i) {
    ...
    if(arg=="--bbox") {
      if(i+4>=args.size()) throw eg::ValidationError("--bbox requires W S E N");
      bbox={ParseDouble(args[++i],arg),ParseDouble(args[++i],arg),ParseDouble(args[++i],arg),ParseDouble(args[++i],arg)}; have_bbox=true;
    } else if(arg=="--grid-spacing-deg") spacing=ParseDouble(RequireValue(args,i,arg),arg);
    else if(arg=="--model-dir"||arg=="--model-directory") model_directory=RequireValue(args,i,arg);
    else if(arg=="--output") output=RequireValue(args,i,arg);
    else if(arg=="--overwrite") overwrite=true;
    else if(arg=="--json"||arg=="--verbose") {}
    else throw eg::ValidationError("unknown prepare-tpxo-cache option: "+arg);
  }
  if(!have_bbox||model_directory.empty()||output.empty()) throw eg::ValidationError("prepare-tpxo-cache requires --bbox, --model-dir, and --output");
  PrintJson(eg::PrepareTpxo10Cache(model_directory,bbox,spacing,output,overwrite));
  return 0;
}
...
int Merge(const std::vector<std::string>& args) {
  ...
  for (std::size_t i = 1; i < args.size(); ++i) {
    if (args[i] == "--input") {
      ...
      if (equals == std::string::npos || equals == 0 || equals + 1 == value.size()) throw eg::ValidationError("--input must be LABEL=PATH");
      inputs.emplace_back(value.substr(0, equals), value.substr(equals + 1));
    } else if (args[i] == "--output") output = RequireValue(args, i, args[i]);
    else if (args[i] == "--overwrite") overwrite = true;
    else throw eg::ValidationError("unknown merge option: " + args[i]);
  }
  if (output.empty()) throw eg::ValidationError("merge-gribs requires --output");
  ...
  for (const auto& [label, count] : merged.input_message_counts) result["input_message_counts"][label] = Json::UInt64(count);
...
void Usage() {
  std::cerr << "Environmental GRIB Generator C++\n"
            << "Commands:\n"
            << "  providers\n  weather-providers\n  generate [options]\n"
