...
constexpr const char* kTokenUrl =
    "https://auth.marine.copernicus.eu/realms/MIS/protocol/openid-connect/"
    "token";
...
bool ValidateCredentialsImpl(const std::string& username,
  ...
  std::unique_ptr<CURL, decltype(&curl_easy_cleanup)> curl(curl_easy_init(),
                                                           curl_easy_cleanup);
  if (!curl)
    throw ValidationError("creating Copernicus authentication request failed");
  char* escaped_user =
      curl_easy_escape(curl.get(), username.c_str(), username.size());
  char* escaped_password =
      curl_easy_escape(curl.get(), password.c_str(), password.size());
  ...
  const std::string form =
      std::string("client_id=toolbox&grant_type=password&username=") +
...
Axis ReadAxis(const Json::Value& asset, const char* name) {
  ...
  if (result.size < 1 || result.step <= 0.0)
    throw ValidationError(std::string("invalid Copernicus ") + name +
                          " axis metadata");
  return result;
}
...
std::vector<std::int16_t> DecodeInt16Blosc(
  ...
  if (compressed.empty())
    throw ValidationError("Copernicus Zarr chunk is empty");
  std::vector<std::int16_t> values(expected_values);
  const int decoded = blosc_decompress(compressed.data(), values.data(),
                                       values.size() * sizeof(std::int16_t));
  if (decoded < 0 ||
      static_cast<std::size_t>(decoded) != values.size() * sizeof(std::int16_t))
    throw ValidationError(
        "Copernicus Zarr chunk failed Blosc decompression or had unexpected "
        "size");
...
std::vector<double> DecodeComponent(BinaryDownload& download,
                                    ...
                                    std::size_t source_points, double scale,
                                    double offset, std::int16_t fill) {
  const auto bytes =
      Get(download,
          root + "/" + variable + "/" + std::to_string(time_index) + ".0.0",
          request);
  const auto packed = DecodeInt16Blosc(bytes, source_points);
  std::vector<double> result(source_points,
...
std::pair<double, bool> Bilinear(const std::vector<double>& values,
  ...
  if (fx < 0.0 || fy < 0.0 || fx > lon.size - 1 || fy > lat.size - 1)
    return {0.0, false};
  ...
  const std::array<std::size_t, 4> index{y0 * lon.size + x0, y0 * lon.size + x1,
                                         ...
                                         y1 * lon.size + x1};
  for (auto i : index)
    if (!std::isfinite(values[i])) return {0.0, false};
...
CopernicusResult GenerateCopernicusNws(
  ...
  if (request.provider != "copernicus_nws")
    throw UnsupportedSourceError(
        "native Copernicus provider currently supports copernicus_nws only");
  if (request.hours < 0 || request.step_hours <= 0)
    throw ValidationError(
        "Copernicus hours must be non-negative and step must be positive");
  ...
  if (!download)
    download = BinaryDownload(
        MakeRetryingHttpGet({}, "Copernicus NWS current", {}, {5, 1000, 8000}));
  ...
  const auto dataset = DiscoverArcoDataset(kProduct, kDataset, request.username,
                                           download, request.timeout_seconds);
  const auto& item = dataset.item;
  const auto& asset = item["assets"]["timeChunked"];
  const std::string root = asset["href"].asString();
  if (root.empty())
    throw ValidationError("Copernicus dataset has no timeChunked Zarr asset");
  ...
  auto currents =
      ParallelMapOrdered(requested_times, 2, [&](const TimePoint& instant) {
        const auto milliseconds =
            std::chrono::duration_cast<std::chrono::milliseconds>(
                instant.time_since_epoch())
                .count();
        ...
        for (std::size_t y = 0; y < grid.ny(); ++y) {
          for (std::size_t x = 0; x < grid.nx(); ++x) {
            const auto ui = Bilinear(u, latitude, longitude, grid.latitudes[y],
                                     grid.longitudes[x]);
            const auto vi = Bilinear(v, latitude, longitude, grid.latitudes[y],
                                     grid.longitudes[x]);
        ...
        if (std::none_of(current.mask.begin(), current.mask.end(),
...
CopernicusResult GenerateCopernicusArcoCurrent(
  ...
  if (request.provider != expected_provider)
    throw UnsupportedSourceError(
        provider_label + " generator requires provider " + expected_provider);
  if (request.hours < 0 || request.step_hours <= 0)
    throw ValidationError(
        "Copernicus hours must be non-negative and step must be positive");
  if (request.username.empty() || request.password.empty())
  ...
  if (!download)
    download = BinaryDownload(
        MakeRetryingHttpGet({}, provider_label, {}, {5, 1000, 8000}));
  if (!validate_credentials) validate_credentials = ValidateCredentialsImpl;
  if (!validate_credentials(request.username, request.password,
                            request.timeout_seconds))
    throw ValidationError("invalid Copernicus username or password");
  const auto dataset =
      DiscoverArcoDataset(product_id, dataset_id, request.username, download,
                          request.timeout_seconds);
  CopernicusResult result{request.output,
                          0,
                          0,
  ...
  const auto fields =
      ReadArcoFields(dataset, {"uo", "vo"}, request.bbox, times, grid,
                     request.username, download, request.timeout_seconds);
  ...
  for (std::size_t t = 0; t < times.size(); ++t) {
    ...
    for (std::size_t i = 0; i < grid.size(); ++i) {
      const bool u_missing =
          !fields.at("uo")[t].mask.empty() && fields.at("uo")[t].mask[i];
      const bool v_missing =
          !fields.at("vo")[t].mask.empty() && fields.at("vo")[t].mask[i];
...
CopernicusResult GenerateCopernicusMediterranean(
    ...
    CredentialValidator validate_credentials) {
  return GenerateCopernicusArcoCurrent(
      request, "copernicus_mediterranean", "Copernicus Mediterranean current",
