...
zip_t* OpenZipArchive(const std::filesystem::path& path, int flags) {
  ...
  zip_source_t* source = zip_source_win32w_create(path.c_str(), 0, -1, &error);
  if (!source) {
...
std::vector<std::byte> ReadZipMember(zip_t* archive,
  ...
  if (!file)
    throw ValidationError("could not open TPXO cache member " + member);
...
NpyArray ParseNpy(const std::vector<std::byte>& bytes,
                  const std::string& member) {
  static constexpr std::array<std::uint8_t, 6> magic{0x93, 'N', 'U',
                                                     'M',  'P', 'Y'};
  if (bytes.size() < 10 ||
  ...
  if (major == 1) {
    ...
    if (bytes.size() < 12)
      throw ValidationError("truncated NPY header in " + member);
    prefix = 12;
  ...
  if (header.find("'fortran_order': True") != std::string::npos) {
    throw ValidationError("Fortran-order NPY arrays are unsupported in " +
                          member);
  }
  if (!std::regex_search(header, match,
                         std::regex("'shape'\\s*:\\s*\\(([^)]*)\\)"))) {
  ...
  result.data.assign(
      bytes.begin() + static_cast<std::ptrdiff_t>(prefix + header_size),
      bytes.end());
  return result;
}
...
std::vector<std::byte> NpyBytes(const std::string& descriptor,
  ...
  std::string header =
      "{'descr': '" + descriptor +
      "', 'fortran_order': False, 'shape': " + shape_text.str() + ", }";
  ...
  result[8] = static_cast<std::byte>(length & 0xff);
  result[9] = static_cast<std::byte>((length >> 8) & 0xff);
  std::memcpy(result.data() + prefix, header.data(), header.size());
  if (byte_count)
    std::memcpy(result.data() + prefix + header.size(), data, byte_count);
  return result;
}
...
std::vector<std::byte> UnicodeNpy(const std::vector<std::string>& values,
  ...
  return NpyBytes("<U" + std::to_string(width), shape, utf32.data(),
...
void AddZipMember(zip_t* archive, const std::string& name,
  ...
  if (!source ||
      zip_file_add(archive, name.c_str(), source, ZIP_FL_OVERWRITE) < 0) {
    if (source)
  ...
  if (index < 0 ||
      zip_set_file_compression(archive, index, ZIP_CM_DEFLATE, 6) < 0)
    throw ValidationError("could not configure TPXO cache compression for " +
                          name);
}
...
std::vector<std::complex<double>> ReadC128(const NpyArray& array,
  ...
  if (array.descriptor != "<c16" && array.descriptor != "=c16") {
    throw ValidationError(name +
                          " must be a little-endian complex128 NPY array");
  }
  const std::size_t count = Product(array.shape);
...
std::vector<std::string> ReadUnicode(const NpyArray& array,
                                     const std::string& name) {
  std::smatch match;
  if (!std::regex_match(array.descriptor, match,
                        std::regex("[<|=]U([0-9]+)"))) {
...
const std::map<std::string, ConstituentParameters>& Parameters() {
  static const std::map<std::string, ConstituentParameters> values{
      // Constant operational reference-level coefficient used by derived XTD
      // Chart-Datum patches.  It is not an astronomical constituent.
      {"z0", {0.0, 0.0}},
      {"m2", {1.731557546, 1.405189e-4}},
      {"s2", {0.0, 1.454441e-4}},
      {"k1", {0.173003674, 7.292117e-5}},
      {"o1", {1.558553872, 6.759774e-5}},
      {"n2", {6.050721243, 1.378797e-4}},
      ...
      {"rho1", {5.254133027, 6.531174e-5}},
      {"mf", {1.756042456, 0.053234e-4}},
      {"mm", {1.964021610, 0.026392e-4}},
...
double MajorPrediction(const Harmonics& values, double tide_days,
  ...
  for (const auto& [name, z] : values) {
    ...
    const double theta =
        86400.0 * found->second.omega * tide_days + found->second.phase + u;
...
std::optional<std::complex<double>> Get(const Harmonics& h,
  ...
  return it == h.end() ? std::nullopt : std::optional(it->second);
}
...
double MinorPrediction(const Harmonics& h, const Astronomy& a) {
  static const std::array<std::string, 9> needed{"q1", "o1", "p1", "k1", "n2",
                                                 "m2", "s2", "k2", "2n2"};
  int available = 0;
  for (const auto& c : needed) available += h.contains(c);
  ...
  const double sn = std::sin(n), cn = std::cos(n), s2n = std::sin(2 * n),
               c2n = std::cos(2 * n);
  std::array<double, 18> f{};
  f.fill(1.0);
  std::array<double, 18> u{};
  u.fill(0.0);
  f[0] = std::hypot(1 + .189 * cn - .0058 * c2n, .189 * sn - .0058 * s2n);
  f[1] = f[2] = f[0];
  f[3] = std::hypot(1 + .185 * cn, .185 * sn);
  f[4] = std::hypot(1 + .201 * cn, .201 * sn);
  f[5] = std::hypot(1 + .221 * cn, .221 * sn);
  f[9] = std::hypot(1 + .198 * cn, .198 * sn);
  f[10] = std::hypot(1 + .640 * cn + .134 * c2n, .640 * sn + .134 * s2n);
  f[11] = std::hypot(1 - .0373 * cn, .0373 * sn);
  f[12] = f[13] = f[15] = f[11];
  f[16] = std::hypot(1 + .441 * cn, .441 * sn);
  u[0] = std::atan2(.189 * sn - .0058 * s2n, 1 + .189 * cn - .0058 * s2n);
  u[1] = u[2] = u[0];
  u[3] = std::atan2(.185 * sn, 1 + .185 * cn);
  u[4] = std::atan2(-.201 * sn, 1 + .201 * cn);
  u[5] = std::atan2(-.221 * sn, 1 + .221 * cn);
  u[9] = std::atan2(-.198 * sn, 1 + .198 * cn);
  u[10] = std::atan2(-.640 * sn - .134 * s2n, 1 + .640 * cn + .134 * c2n);
  u[11] = std::atan2(-.0373 * sn, 1 - .0373 * cn);
  u[12] = u[13] = u[15] = u[11];
  ...
  for (std::size_t i = 0; i < MinorDefinitions().size(); ++i) {
    ...
    double g = 0.0;
    for (std::size_t j = 0; j < 7; ++j) g += args[j] * d.coefficients[j];
    const auto& value = z.at(amplitude_name);
    const double theta = g * kPi / 180.0 + u[i];
    result += value.real() * f[i] * std::cos(theta) -
              value.imag() * f[i] * std::sin(theta);
  }
  return result;
}
...
std::vector<CurrentGrid> PredictComponentPair(
  ...
  for (std::size_t t = 0; t < times.size(); ++t) {
    ...
    for (std::size_t p = 0; p < points; ++p) {
      ...
      grid.u_mps[p] = um / 100.0;
      grid.v_mps[p] = vm / 100.0;
    }
...
std::vector<double> PredictAtlasHarmonicGrid(
  ...
  if (point_count == 0 ||
    ...
    throw ValidationError(
        "ATLAS harmonic coefficient dimensions are inconsistent");
  }
  ...
  if (times.size() > std::numeric_limits<std::size_t>::max() / point_count) {
...
std::complex<double> ShiftAtlasHarmonicCoefficient(
  ...
  const auto found = Parameters().find(constituent);
  if (found == Parameters().end())
    throw ValidationError("unsupported ATLAS harmonic constituent: " +
                          constituent);
  if (!std::isfinite(coefficient.real()) ||
      !std::isfinite(coefficient.imag()) ||
      !std::isfinite(prediction_time_shift_seconds))
    throw ValidationError("harmonic coefficient time shift is invalid");
  // z*exp(+i*w*d) predicts h(t+d). To move an event later by d, evaluate the
  // original signal at t-d and therefore rotate by -w*d.
  return coefficient *
         std::polar(1.0, -found->second.omega * prediction_time_shift_seconds);
}
...
TpxoCache LoadTpxoCache(const std::filesystem::path& path) {
  ...
  if (!archive)
    throw ValidationError("could not read TPXO cache file " + PathToUtf8(path));
  try {
    const auto metadata_values =
        ReadUnicode(ReadNpy(archive, "metadata_json"), "metadata_json");
    if (metadata_values.size() != 1)
      throw ValidationError("TPXO cache metadata must be scalar");
    Json::CharReaderBuilder builder;
    Json::Value metadata;
    std::string errors;
    std::istringstream input(metadata_values[0]);
    if (!Json::parseFromStream(builder, input, &metadata, &errors))
      throw ValidationError("invalid TPXO cache metadata JSON");
    const int format_version = metadata["format_version"].asInt();
    if (metadata["format"].asString() !=
            "tidal-current-grib-generator-tpxo-cache" ||
        format_version != 1)
    ...
    if (format_version == 1 && metadata["pyTMD_version"].isNull() &&
      ...
      throw ValidationError(
          "legacy native TPXO cache has ambiguous current units and must be "
          "rebuilt");
    }
    ...
    const auto longitudes =
        ReadF64(ReadNpy(archive, "longitudes"), "longitudes");
    const auto latitudes = ReadF64(ReadNpy(archive, "latitudes"), "latitudes");
    const auto constituents =
        ReadUnicode(ReadNpy(archive, "constituents"), "constituents");
    const auto ua = ReadNpy(archive, "u_complex"),
               va = ReadNpy(archive, "v_complex");
    auto u = ReadC128(ua, "u_complex"), v = ReadC128(va, "v_complex");
    if (ua.shape != va.shape || ua.shape.size() != 2 ||
        ua.shape[0] != constituents.size() ||
        ua.shape[1] != latitudes.size() * longitudes.size())
      throw ValidationError(
          "TPXO cache harmonic array dimensions do not match its grid");
    const auto& b = metadata["bbox"];
    TpxoCache cache{
        metadata,
        {b["west"].asDouble(), b["south"].asDouble(), b["east"].asDouble(),
         b["north"].asDouble()},
...
void WriteTpxoCache(const std::filesystem::path& path, const TpxoCache& cache,
  ...
  if (std::filesystem::exists(path) && !overwrite)
    throw ValidationError(
        "TPXO cache already exists; enable overwrite to replace it");
  if (cache.u_cm_s.size() != cache.constituents.size() * cache.grid.size() ||
      cache.v_cm_s.size() != cache.u_cm_s.size())
  ...
  if (!archive)
    throw ValidationError("could not create TPXO cache " +
  ...
  try {
    ...
    AddZipMember(archive, "longitudes.npy",
                 NpyBytes("<f8", {cache.grid.longitudes.size()},
                          cache.grid.longitudes.data(),
                          cache.grid.longitudes.size() * sizeof(double)));
    AddZipMember(archive, "latitudes.npy",
                 NpyBytes("<f8", {cache.grid.latitudes.size()},
                          cache.grid.latitudes.data(),
                          cache.grid.latitudes.size() * sizeof(double)));
    std::vector<double> lon_points, lat_points;
    lon_points.reserve(cache.grid.size());
    lat_points.reserve(cache.grid.size());
    for (double lat : cache.grid.latitudes)
      for (double lon : cache.grid.longitudes) {
        lon_points.push_back(lon);
        lat_points.push_back(lat);
      }
    AddZipMember(archive, "lon_points.npy",
                 NpyBytes("<f8", {lon_points.size()}, lon_points.data(),
                          lon_points.size() * sizeof(double)));
    AddZipMember(archive, "lat_points.npy",
                 NpyBytes("<f8", {lat_points.size()}, lat_points.data(),
                          lat_points.size() * sizeof(double)));
    AddZipMember(archive, "constituents.npy",
                 UnicodeNpy(cache.constituents, {cache.constituents.size()}));
    const std::vector<std::size_t> harmonic_shape{cache.constituents.size(),
                                                  cache.grid.size()};
    AddZipMember(archive, "u_complex.npy",
                 NpyBytes("<c16", harmonic_shape, cache.u_cm_s.data(),
                          cache.u_cm_s.size() * sizeof(std::complex<double>)));
    AddZipMember(archive, "v_complex.npy",
                 NpyBytes("<c16", harmonic_shape, cache.v_cm_s.data(),
                          cache.v_cm_s.size() * sizeof(std::complex<double>)));
    if (zip_close(archive) != 0) {
      archive = nullptr;
      throw ValidationError("could not finalize TPXO cache " +
                            PathToUtf8(temporary));
    }
    archive = nullptr;
...
Json::Value InspectTpxoCache(const std::filesystem::path& path) {
  ...
  Json::Value value = cache.metadata;
  value["input_cache"] = PathToUtf8(path);
  value["grid_size"]["nx"] = Json::UInt64(cache.grid.nx());
  value["grid_size"]["ny"] = Json::UInt64(cache.grid.ny());
  value["point_count"] = Json::UInt64(cache.grid.size());
  value["valid"] = true;
  return value;
}
#endif
...
TpxoGenerationResult GenerateFromTpxoCache(
  ...
  const auto fields = PredictTpxoCache(
      cache, BuildTimeSequence(start, hours, step_hours), infer_minor);
  WriteGrib1Currents(fields, output);
  const auto messages = ScanGribMessages(output);
  return {output, messages.message_count, std::filesystem::file_size(output),
          InspectGrib(output)};
}
#endif
