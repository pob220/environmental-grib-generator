...
std::vector<std::string> MetadataRoots(BinaryDownload& download,
  ...
  try {
    const auto config = Parse(download(kClientConfig, std::min(timeout, 10.0)),
...
std::string Suffix(const std::string& username) {
  std::string result =
      "?x-cop-client=environmental-grib-generator&x-cop-client-version=0.1.11";
  if (!username.empty()) {
    result += "&x-cop-user=";
...
Axis AxisFor(const Json::Value& asset, const std::string& variable,
  ...
  Axis axis;
  axis.size =
      coords.isMember("len") ? coords["len"].asUInt64() : dim["len"].asUInt64();
...
std::vector<unsigned char> Decompress(const std::vector<unsigned char>& input) {
  ...
  const int decoded =
      blosc_decompress(input.data(), output.data(), output.size());
...
public:
  VariableReader(const ArcoDataset& dataset, std::string variable,
    ...
    const auto& metadata =
        dataset.item["properties"]["cube:variables"][variable_];
    for (const auto& dimension : metadata["dimensions"])
      dimensions_.push_back(dimension.asString());
    latitude_ = AxisFor(asset, variable_, "latitude");
    longitude_ = AxisFor(asset, variable_, "longitude");
    time_ = AxisFor(asset, variable_, "time");
    if (asset["viewDims"].isMember("elevation"))
      elevation_ = AxisFor(asset, variable_, "elevation");
  ...
  NetCDFScalarField Read(TimePoint instant, const RegularGrid& grid) {
    ...
    NetCDFScalarField result{instant,
                             variable_,
                             variable_,
                             "",
...
private:
  ...
  std::pair<double, bool> At(std::size_t time_index, std::size_t y,
    ...
    for (const auto& dimension : dimensions_) {
      if (dimension == "time")
        chunk_indices.push_back(tc);
      else if (dimension == "latitude")
        chunk_indices.push_back(yc);
      else if (dimension == "longitude")
        chunk_indices.push_back(xc);
      else if (dimension == "elevation" || dimension == "depth")
        chunk_indices.push_back(0);
      else
        throw ValidationError("unsupported ARCO variable dimension: " +
                              dimension);
    }
    std::ostringstream key;
    for (std::size_t i = 0; i < chunk_indices.size(); ++i) {
      if (i) key << '.';
      key << chunk_indices[i];
    }
    auto found = cache_.find(key.str());
    if (found == cache_.end()) {
      const auto bytes = download_(
          root_ + "/" + variable_ + "/" + key.str() + Suffix(username_),
          timeout_);
      found = cache_.emplace(key.str(), Decompress(bytes)).first;
    }
    const std::size_t time_count =
        std::min(time_.chunk, time_.size - tc * time_.chunk);
    const std::size_t lat_count =
        std::min(latitude_.chunk, latitude_.size - yc * latitude_.chunk);
    const std::size_t lon_count =
        std::min(longitude_.chunk, longitude_.size - xc * longitude_.chunk);
    const std::size_t elevation_count =
        elevation_ ? std::min(elevation_->chunk, elevation_->size) : 1;
    std::map<std::string, std::size_t> local{
        {"time", time_index % time_.chunk},
        {"latitude", y % latitude_.chunk},
        {"longitude", x % longitude_.chunk},
        {"elevation", 0},
        {"depth", 0}};
    std::map<std::string, std::size_t> sizes{{"time", time_count},
                                             {"latitude", lat_count},
                                             {"longitude", lon_count},
    ...
    if (dtype_ == "<i2") {
      if ((flat + 1) * 2 > raw.size())
        throw ValidationError("ARCO int16 chunk index is out of bounds");
      ...
      if ((flat + 1) * 4 > raw.size())
        throw ValidationError("ARCO float32 chunk index is out of bounds");
      const auto packed = Little<float>(raw.data() + flat * 4);
      if (!std::isfinite(packed) || packed == static_cast<float>(missing_))
        return {0.0, false};
      value = packed * scale_ + offset_;
    } else
      throw ValidationError("unsupported ARCO dtype: " + dtype_);
    return {value, true};
  }
...
ArcoDataset DiscoverArcoDataset(const std::string& product,
  ...
  for (const auto& root : MetadataRoots(download, timeout)) {
    try {
      const auto product_json =
          Parse(download(root + "/" + product + "/product.stac.json" +
                             Suffix(username),
                         attempt_timeout),
                "STAC product");
      ...
      if (matches.empty()) throw ValidationError("dataset is not listed");
      std::sort(matches.begin(), matches.end());
      const auto item = Parse(
          download(DatasetUrl(root, product, matches.back()) + Suffix(username),
                   attempt_timeout),
          "STAC dataset");
...
std::map<std::string, std::vector<NetCDFScalarField>> ReadArcoFields(
  ...
  struct VariableFields {
    std::string variable;
    std::vector<NetCDFScalarField> fields;
  };
  auto generated = ParallelMapOrdered(
      variables, kDefaultDownloadConcurrency, [&](const std::string& variable) {
        ...
        for (const auto instant : times)
          fields.fields.push_back(reader.Read(instant, target_grid));
        return fields;
      });
  for (auto& variable : generated) {
    result.emplace(std::move(variable.variable), std::move(variable.fields));
  }
  (void)bbox;
  return result;
}
